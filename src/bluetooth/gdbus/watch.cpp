							...
							const char *argument)
{
	...
	for (current = listeners;
		...
		struct filter_data *data = (filter_data*)current->data;
						...
						guint id)
{
	...
	for (l = data->callbacks; l; l = l->next) {
		struct filter_callback *cb =  (struct filter_callback*)l->data;
		if (cb->id == id)
			return cb;
	}
...
static DBusHandlerResult signal_filter(DBusConnection *connection,
					DBusMessage *message, void *user_data)
{
	struct filter_data *data = (filter_data*) user_data;
	struct filter_callback *cb;

	while (data->callbacks) {
		cb = (filter_callback*) data->callbacks->data;
...
static void update_name_cache(const char *name, const char *owner)
{
	...
	for (l = listeners; l != NULL; l = l->next) {
		struct filter_data *data = (struct filter_data*)l->data;
...
static DBusHandlerResult service_filter(DBusConnection *connection,
					DBusMessage *message, void *user_data)
{
	struct filter_data *data = (struct filter_data*) user_data;
	struct filter_callback *cb;
	char *name, *old, *neww;
	...
	if (!dbus_message_get_args(message, NULL,
				...
				DBUS_TYPE_STRING, &neww,
				DBUS_TYPE_INVALID)) {
		error("Invalid arguments for NameOwnerChanged signal");
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	}

	update_name_cache(name, neww);

	while (data->callbacks) {
		cb = (struct filter_callback*)data->callbacks->data;
					...
					DBusMessage *message, void *user_data)
{
	...
	for (current = listeners; current != NULL; current = current->next) {
		data = (struct filter_data*) current->data;
	...
	for (current = delete_listener; current != NULL;
		...
		GSList *l = (GSList*) current->data;

		data = (struct filter_data*) l->data;
...
{
	struct service_data *data = (service_data*) user_data;
...
{
	struct service_data *data = (struct service_data*)user_data;
	DBusMessage *reply;
	DBusError err;
					...
					struct filter_callback *callback)
{
	...
	data = (struct service_data*) g_try_malloc0(sizeof(*data));
	if (data == NULL) {
