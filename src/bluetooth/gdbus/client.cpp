...
{
	struct prop_entry *prop = (prop_entry*) data;
				...
				DBusMessageIter *iter, gboolean send_changed)
{
	...
	prop = (prop_entry*) g_hash_table_lookup(proxy->prop_list, name);
	if (prop != NULL) {
...
{
	struct refresh_property_data *data = (struct refresh_property_data*)user_data;
...
{
	struct set_property_data *data = (struct set_property_data*) user_data;
	DBusMessage *reply = dbus_pending_call_steal_reply(call);
	DBusError error;
...
{
	struct method_call_data *data = (struct method_call_data*)user_data;
	DBusMessage *reply = dbus_pending_call_steal_reply(call);
...
static gboolean interfaces_added(DBusConnection *conn, DBusMessage *msg,
							void *user_data)
{
	GDBusClient *client = (GDBusClient*) user_data;
	DBusMessageIter iter;
	const char *path;
					...
					const char *service, const char *path)
{
	...
	for (i = 0; i < client->match_rules->len; i++) {
		modify_match(client->dbus_conn, "AddMatch",
				(const char*)g_ptr_array_index(client->match_rules, i));
	}
